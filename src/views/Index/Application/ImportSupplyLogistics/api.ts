...
export const getDetail = DefaultGenerator.post<{ 'service_id': number }, SupplyLogistics>('/master/supply_logistics_import/applyInfo');

/**
 * 加急
 */
export const expedite = DefaultGenerator.post<{ 'service_id': number }>('/master/supply_logistics_import/expedited');

export const cancel = DefaultGenerator.post<{ 'service_id': number }>('/master/supply_logistics_import/revoke');

/**
 * 申请完成
 */
export const complete = DefaultGenerator.post<{ 'service_id': number }>('/master/supply_logistics_import/applySuccess');

/**
 * 判断是否有正在办理的货源共享业务
 */
export const preCheck = DefaultGenerator.post('/master/supply_logistics_import/judge');

export const save1 = DefaultGenerator.post<BaseSupplyLogistics, SupplyLogistics>('/master/supply_logistics_import/saveInfo');

export const next1 = DefaultGenerator.post<BaseSupplyLogistics, SupplyLogistics>('/master/supply_logistics_import/applySupplyLogistics');

export const save2 = DefaultGenerator.post<
  { 'service_id': number; 'warehouse': BaseWarehouse[] },
  SupplyLogistics
>('/master/supply_logistics_import/saveWarehouse', {
  data: {
    next_step: 0,
  },
});
...
>('/master/supply_logistics_import/getWarehouseList');

export const save3 = DefaultGenerator.post<
  Pick<SupplyLogistics, 'service_id' | 'route'>,
  SupplyLogistics
>('/master/supply_logistics_import/saveRoute', {
  data: {
    next_step: 0,
  },
});
